...
set -ex
## Check ENV
[[ ! "$MANGO_BENCHER_DIR" ]]&& MANGO_BENCHER_DIR=$HOME/mango_bencher
...
echo MANGO_BENCHER_DIR: $MANGO_BENCHER_DIR >> env.output
...
cd $HOME
[[ -d "$MANGO_BENCHER_DIR" ]]&& rm -rf mango_bencher
# clone mango_bencher and mkdir dep dir
git clone $MANGO_BENCHER_REPO
if [[ ! -d "$MANGO_BENCHER_DIR" ]];then
    exit 1
fi

cd $MANGO_BENCHER_DIR
git submodule init 
...
if  [[ "$BUILD_MANGO_BENCHER" == "true" ]];then
	# cp from MANGO_BENCHER_DIR to HOME
	cp $MANGO_BENCHER_DIR/target/release/solana-bench-mango $HOME
...
upload_file $MANGO_BENCHER_DIR/target/release/solana-bench-mango
